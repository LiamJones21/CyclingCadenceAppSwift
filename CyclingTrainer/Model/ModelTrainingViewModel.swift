...
//    override init() {
//        super.init()
//        print("ModelTrainingViewModel initialized")
//        setupSession()
//        setupAdvertiser()
//        loadLocalSessions()
//        loadLocalModels()
...
//
//    deinit {
//        print("ModelTrainingViewModel deinitialized")
//    }
//
...
//        print("Setting up MCNearbyServiceAdvertiser")
//        advertiser = MCNearbyServiceAdvertiser(peer: myPeerID, discoveryInfo: nil, serviceType: serviceType)
//        advertiser.delegate = self
//        advertiser.startAdvertisingPeer()
//    }
...
//    @Published var bestAccuracy: Double?
//    @Published var connectedPeers: [MCPeerID] = []
//    @Published var trainingSettings: [String: Any] = [:]
//    
...
//        invitationHandler(true, session) // Accept the invitation
//    }
//    
//    func advertiser(_ advertiser: MCNearbyServiceAdvertiser, didNotStartAdvertisingPeer error: Error) {
//        print("Failed to start advertising: \(error.localizedDescription)")
...
//            // Handle the data as per your application's logic
//            if let message = try? JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
//               let type = message["type"] as? String {
//                switch type {
//                case "trainingSettingsUpdate":
...
//    }
//
//    // Other methods as per your application logic
//}


// ModelTrainingViewModel.swift (for Mac)
import Foundation
import Combine
import MultipeerConnectivity
import CoreML
...
class ModelTrainingViewModel: NSObject, ObservableObject {
    ...
    @Published var connectedPeers: [MCPeerID] = []
    @Published var trainingSettings: [String: Any] = [:]
    @Published var phoneSessions: [Session] = []
    @Published var phoneModels: [ModelConfig] = []
    ...
    func loadLocalSessions() {
        ...
        if let data = try? Data(contentsOf: fileURL),
           let sessions = try? JSONDecoder().decode([Session].self, from: data) {
            ...
            print("No sessions found or failed to decode.")
        }
    }
    ...
    func saveLocalSessions() {
        ...
        if let data = try? JSONEncoder().encode(sessions) {
            do {
                try data.write(to: fileURL)
                print("Saved \(sessions.count) sessions to file.")
            } catch {
    ...
    func loadLocalModels() {
        ...
        if let data = try? Data(contentsOf: fileURL),
           let models = try? JSONDecoder().decode([ModelConfig].self, from: data) {
            self.models = models
            print("Loaded \(models.count) models from file.")
        } else {
            print("No models found or failed to decode.")
        }
    }
    ...
    func saveLocalModels() {
        ...
        if let data = try? JSONEncoder().encode(models) {
            do {
                try data.write(to: fileURL)
                print("Saved \(models.count) models to file.")
            } catch {
                print("Error saving models: \(error.localizedDescription)")
            }
        } else {
    ...
    func sendModelsList(to peerID: MCPeerID) {
        let modelList = models.map { $0.toDictionary() }
        let message: [String: Any] = ["type": "modelList", "models": modelList]
        sendMessage(message: message, to: peerID)
    }

    func sendModel(model: ModelConfig, to peerID: MCPeerID) {
        guard let modelURL = getCompiledModelURL(modelName: model.name) else {
            print("Compiled model not found for \(model.name)")
            return
        }
...
// MARK: - MCSessionDelegate & MCNearbyServiceAdvertiserDelegate Methods

extension ModelTrainingViewModel: MCSessionDelegate, MCNearbyServiceAdvertiserDelegate {
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        DispatchQueue.main.async {
            switch state {
            case .connected:
           ...
           let type = message["type"] as? String {
            ...
            case "sessionList":
                if let sessionsArray = message["sessions"] as? [[String: Any]] {
                    ...
                    DispatchQueue.main.async {
                        self.phoneSessions = receivedSessions
                        print("Received sessions from \(peerID.displayName)")
                    }
                }
            case "requestModelList":
                self.sendModelsList(to: peerID)
            case "modelList":
                if let modelsArray = message["models"] as? [[String: Any]] {
                    let receivedModels = modelsArray.compactMap { ModelConfig.fromDictionary($0) }
                    DispatchQueue.main.async {
                        self.phoneModels = receivedModels
                        print("Received models from \(peerID.displayName)")
                    }
                }
            case "requestModel":
                if let modelName = message["modelName"] as? String {
                    if let model = self.models.first(where: { $0.name == modelName }) {
                        self.sendModel(model: model, to: peerID)
                    }
                }
            case "modelData":
                if let modelName = message["modelName"] as? String {
            ...
            // Assume it's model data
            handleReceivedModel(data: data)
        }
    }

    func handleReceivedModel(data: Data) {
